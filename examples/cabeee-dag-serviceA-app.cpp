...
public:
  ...
  run(char* PREFIX, char* servicePrefix, char* makespanNS)
  {
    ...
    m_face.setInterestFilter(fullPrefix,
                             ...
                             std::bind(&ServiceA::onRegisterFailed, this, _1, _2));
...
private:
  ...
  sendInterest(const std::string& interestName, std::string dagString)
  {
    ...
    //std::cout << "ServiceA: Sending Interest packet for " << interest << std::endl;

    m_face.expressInterest(interest,
                           std::bind(&ServiceA::onData, this,  _1, _2),
                           std::bind(&ServiceA::onNack, this, _1, _2),
                           std::bind(&ServiceA::onTimeout, this, _1));
...
private:
  ...
  onInterest(const Interest& interest)
  {
    ...
    if (m_done == true)
    {
      //std::cout << "    ServiceA: We already ran this service before. Responding with internally stored result!" << std::endl;
      // send stored result
...
{
  try {
    ndn::examples::ServiceA serviceA;
    serviceA.run(argv[1], argv[2], argv[3]);
    return 0;
  }
