...
sleep ${sleep}; ssh ${username}@${rtr1WiFiIP} "nfdc face create remote ether://[${producerETHMAC}] local dev://${rtr1USBETHinterface} persistency permanent >/dev/null 2>&1 &"
sleep ${sleep}; ssh ${username}@${producerWiFiIP} "nfdc face create remote ether://[${rtr1USBETHMAC}] local dev://${producerETHinterface} persistency permanent >/dev/null 2>&1 &"
