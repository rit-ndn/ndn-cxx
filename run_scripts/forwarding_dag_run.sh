...
do
	...
	do
		...
		ssh ${username}@${rpi5producerWiFiIP} "~/ndn/ndn-cxx/run_scripts/forwarding_dag_run_local.sh producer ${scenario} ${sleep} ${changeLinkDelay} ${linkDelayMS} >/dev/null 2>&1 &"
		ssh ${username}@${rpi5rtr1WiFiIP}     "~/ndn/ndn-cxx/run_scripts/forwarding_dag_run_local.sh rtr1     ${scenario} ${sleep} ${changeLinkDelay} ${linkDelayMS} >/dev/null 2>&1 &"
		ssh ${username}@${rpi5rtr2WiFiIP}     "~/ndn/ndn-cxx/run_scripts/forwarding_dag_run_local.sh rtr2     ${scenario} ${sleep} ${changeLinkDelay} ${linkDelayMS} >/dev/null 2>&1 &"
		ssh ${username}@${rpi5rtr3WiFiIP}     "~/ndn/ndn-cxx/run_scripts/forwarding_dag_run_local.sh rtr3     ${scenario} ${sleep} ${changeLinkDelay} ${linkDelayMS} >/dev/null 2>&1 &"
		...
		cmd="$HOME/ndn/ndn-cxx/run_scripts/forwarding_dag_run_local.sh consumer ${scenario} ${sleep} ${changeLinkDelay} ${linkDelayMS}"
